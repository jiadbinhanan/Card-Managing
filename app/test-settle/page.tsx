     ...
     try {
        ...
        for (const profile of profiles) {
           ...
           if (cleanPhone.length >= 10) {
              ...
              console.log(`Sending rotation_settlement_alert to ${profile.name}`, settlementComponents);
              await fetch('/api/send-whatsapp', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ phone: cleanPhone, templateName: "rotation_settlement_alert", components: settlementComponents })
              });
           }
        }
